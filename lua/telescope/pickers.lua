...
local ns_telescope_group_by_headers = api.nvim_create_namespace "telescope_group_by_headers"
...
function Picker:clear_extra_rows(results_bufnr)
  ...
  if self.sorting_strategy == "ascending" then
    ...
    ok, msg = pcall(api.nvim_buf_set_lines, results_bufnr, worst_line + self:_group_by_offset(), -1, false, {})

    if self.group_by ~= nil then
      local begin = num_results == 0 and 0 or num_results + self:_group_by_offset()
      pcall(api.nvim_buf_clear_namespace, results_bufnr, ns_telescope_group_by_headers, begin, -1)
    end
  else
...
function Picker:find()
  ...
  if self.results_bufnr and api.nvim_buf_is_valid(self.results_bufnr) then
    api.nvim_buf_clear_namespace(self.results_bufnr, ns_telescope_group_by_headers, 0, -1)
  end
...
function Picker:set_selection(row)
  ...
  then
    api.nvim_win_set_cursor(self.results_win, { row, 0 })
  end
...
function Picker:_set_headers(index, entry)
  ...
  if above == nil then
    api.nvim_buf_clear_namespace(self.results_bufnr, ns_telescope_group_by_headers, row - 1, row + 1)
    api.nvim_buf_set_extmark(self.results_bufnr, ns_telescope_group_by_headers, row - 1, 0, {
  ...
  elseif self.group_by._resolved_group_getter(entry) ~= self.group_by._resolved_group_getter(above) then
    api.nvim_buf_set_extmark(self.results_bufnr, ns_telescope_group_by_headers, row, 0, {
  ...
  if below ~= nil then
    ...
    api.nvim_buf_clear_namespace(self.results_bufnr, ns_telescope_group_by_headers, row + 1, row + 2)
...
function Picker:entry_adder(index, entry, _, insert)
  ...
  if not insert and self.group_by ~= nil then
    api.nvim_buf_clear_namespace(self.results_bufnr, ns_telescope_group_by_headers, row, row + 1)
  end
...
function Picker:get_result_completor(results_bufnr, _, prompt, status_updater)
  return vim.schedule_wrap(function()
    ...
    if self.sorting_strategy == "descending" then
      ...
      api.nvim_win_set_cursor(self.results_win, { self.max_results + self:_group_by_offset() - visible_result_rows, 1 })
      api.nvim_win_set_cursor(self.results_win, { self.max_results + self:_group_by_offset(), 1 })
    end
    self:_on_complete()
  end)
end
...
function pickers.on_close_prompt(prompt_bufnr)
  ...
  if picker.group_by ~= nil then
    api.nvim_buf_clear_namespace(picker.results_bufnr, ns_telescope_group_by_headers, 0, -1)
    if picker.group_by_workaround_applied then
      picker.group_by_workaround_applied = false
      api.nvim_buf_set_lines(picker.results_bufnr, 0, 1, false, {})
    end
    picker.group_by = nil
  end
